...
class TestCourseSearchToolExecute:
    def test_execute_with_results_formats_header_and_body(
    ...
    def test_execute_populates_last_sources_with_correct_text_and_link(
        self, search_tool, mock_vector_store, sample_search_results
    ):
        mock_vector_store.search.return_value = sample_search_results
        ...
        assert search_tool.last_sources == [
            {
                "text": "Test Course: RAG Fundamentals - Lesson 1",
                "link": "https://example.com/l1",
            },
            {
                "text": "Test Course: RAG Fundamentals",
                "link": "https://example.com/course",
            },
        ]
        mock_vector_store.get_lesson_link.assert_called_once_with(
            "Test Course: RAG Fundamentals", 1
        )
        mock_vector_store.get_course_link.assert_called_once_with(
            "Test Course: RAG Fundamentals"
        )

    def test_execute_lesson_link_falls_back_to_course_link_when_lesson_link_is_none(
        self, search_tool, mock_vector_store, sample_search_results
    ):
    ...
    def test_execute_passes_course_name_filter_through_to_search(
        self, search_tool, mock_vector_store, sample_search_results
    ):
        mock_vector_store.search.return_value = sample_search_results
        ...
        mock_vector_store.search.assert_called_once_with(
            query="q", course_name="MCP", lesson_number=None
        )

    def test_execute_passes_lesson_number_filter_through_to_search(
        self, search_tool, mock_vector_store, sample_search_results
    ):
        mock_vector_store.search.return_value = sample_search_results
        ...
        mock_vector_store.search.assert_called_once_with(
            query="q", course_name=None, lesson_number=2
        )

    def test_execute_passes_both_filters_through_to_search(
        self, search_tool, mock_vector_store, sample_search_results
    ):
        mock_vector_store.search.return_value = sample_search_results
    ...
    def test_execute_empty_results_no_filters_message(
    ...
    def test_execute_empty_results_with_course_name_message(
        self, search_tool, mock_vector_store, empty_search_results
    ):
        mock_vector_store.search.return_value = empty_search_results
    ...
    def test_execute_empty_results_with_lesson_number_message(
        self, search_tool, mock_vector_store, empty_search_results
    ):
        mock_vector_store.search.return_value = empty_search_results
    ...
    def test_execute_empty_results_with_both_filters_message(
        self, search_tool, mock_vector_store, empty_search_results
    ):
        mock_vector_store.search.return_value = empty_search_results
    ...
    def test_execute_empty_results_lesson_zero_included_in_message(
        self, search_tool, mock_vector_store, empty_search_results
    ):
        """
    ...
    def test_last_sources_overwritten_not_accumulated_across_calls(
        self, search_tool, mock_vector_store, sample_search_results
    ):
...
class TestCourseOutlineToolExecute:
    def test_execute_outline_found_formats_title_link_and_lessons(
        self, outline_tool, mock_vector_store
    ):
    ...
    def test_execute_outline_not_found_returns_message(
        self, outline_tool, mock_vector_store
    ):
    ...
    def test_execute_outline_missing_course_link_renders_na(
        self, outline_tool, mock_vector_store
    ):
        mock_vector_store.get_course_outline.return_value = {
            "title": "Test Course",
...
class TestToolManager:
    def test_register_tool_adds_tool_by_name_from_definition(
        self, tool_manager, search_tool
    ):
        tool_manager.register_tool(search_tool)

        assert tool_manager.tools["search_course_content"] is search_tool

    def test_get_tool_definitions_returns_list_of_both_registered_tool_schemas(
        self, populated_tool_manager
    ):
    ...
    def test_execute_tool_unknown_name_returns_error_string_not_raise(
        self, tool_manager
    ):
    ...
    def test_execute_tool_dispatches_kwargs_to_correct_tool(
        self, populated_tool_manager, mock_vector_store, sample_search_results
    ):
        mock_vector_store.search.return_value = sample_search_results

        populated_tool_manager.execute_tool(
            "search_course_content", query="q", course_name="X"
        )

        mock_vector_store.search.assert_called_once_with(
            query="q", course_name="X", lesson_number=None
        )

    def test_get_last_sources_returns_sources_from_search_tool_and_skips_outline_tool(
        self, populated_tool_manager, mock_vector_store, sample_search_results
    ):
        mock_vector_store.search.return_value = sample_search_results
        mock_vector_store.get_course_outline.return_value = {
    ...
    def test_get_last_sources_returns_empty_list_when_no_tool_has_sources(
        self, populated_tool_manager
    ):
        assert populated_tool_manager.get_last_sources() == []

    def test_reset_sources_clears_search_tool_last_sources_but_ignores_outline_tool(
        self, populated_tool_manager, mock_vector_store, sample_search_results
    ):
        mock_vector_store.search.return_value = sample_search_results
        populated_tool_manager.execute_tool("search_course_content", query="q")
